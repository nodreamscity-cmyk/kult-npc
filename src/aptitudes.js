...
${conSecreto ? `SECRETO INCONFESABLE: Este PNJ DEBE tener un secreto. Elige el arquetipo más adecuado:
${SECRETOS_KULT.map(s => '· ' + s).join('\n')}
El secreto debe encajar con el perfil y tener peso dramático real.` : 'SECRETO INCONFESABLE: Este nivel no genera secreto. Los campos secreto_inconfesable y secreto_arquetipo deben ser null.'}`
}
  const puntos = amenaza === 'único' ? '120-140 (elige dentro del rango según el concepto)' : PUNTOS_POR_NIVEL[amenaza]
  const edadMax = (amenaza === 'alto' || amenaza === 'muy alto') ? 45 : null
  const sinEnvejecimiento = amenaza === 'único'
