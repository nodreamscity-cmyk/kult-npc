  ...
  return (
    <div className={styles.wrap}>
      ...
      <div className={`${styles.page} ${styles.page1}`}>
        ...
        <div className={styles.identidad}>
          ...
          <div className={styles.metaRow}>
            {[
              ...
              pnj.minoria && ['Minoría', pnj.minoria],
        ...
        <div className={styles.narrativaBlock}>
          <div className={styles.sectionTitle}>Perfil</div>
          <div className={styles.narrGrid}>
            <div className={styles.narrItem}><div className={styles.narrLabel}>Apariencia</div><div className={styles.narrText}>{pnj.apariencia}</div></div>
            <div className={styles.narrItem}><div className={styles.narrLabel}>Personalidad</div><div className={styles.narrText}>{pnj.personalidad}</div></div>
            <div className={`${styles.narrItem} ${styles.full}`}><div className={styles.narrLabel}>Comportamiento ante los jugadores</div><div className={styles.narrText}>{pnj.comportamiento_jugadores}</div></div>
            <div className={styles.narrItem}><div className={styles.narrLabel}>Familiares / Vínculos</div><div className={styles.narrText}>{pnj.familiares}</div></div>
            <div className={styles.narrItem}><div className={styles.narrLabel}>Amigos / Aliados</div><div className={styles.narrText}>{pnj.amigos_vinculos}</div></div>
          </div>
        </div>

        <div className={styles.economiaBlock}>
      ...
      <div className={`${styles.page} ${styles.page2}`}>
        ...
        <div className={styles.habBlock} style={{top:'8mm'}}>
          ...
          {habPrincipal.length > 0 && (
            <div className={styles.bloque}>
              ...
              <div className={`${styles.habGrid} ${styles.twoCol}`}>{habPrincipal.map((h,i) => <HabRow key={i} h={h}/>)}</div>
            </div>
          )}
          {habAncla.length > 0 && (
            <div className={styles.bloque}>
              <div className={styles.bloqueTitle}>Ancla narrativa</div>
              <div className={`${styles.habGrid} ${styles.twoCol}`}>{habAncla.map((h,i) => <HabRow key={i} h={h}/>)}</div>
            </div>
          )}
          {habBasica.length > 0 && (
            <div className={styles.bloque}>
              <div className={styles.bloqueTitle}>Básicas</div>
              <div className={`${styles.habGrid} ${styles.twoCol}`}>{habBasica.map((h,i) => <HabRow key={i} h={h}/>)}</div>
            </div>
          )}
          {am?.disciplina && (
            <div className={styles.bloque}>
              ...
              <div className={`${styles.habGrid} ${styles.twoCol}`}>
                ...
                {(am.maniobras||[]).filter(m=>m.valor).map((h,i) => <HabRow key={i} h={h}/>)}
              </div>
            </div>
          )}
        </div>

        <div className={styles.habBlock} style={{top:'155mm'}}>
          ...
          <div className={styles.vdGrid}>
            <div>
              ...
              {(pnj.ventajas||[]).length > 0
                ? pnj.ventajas.map((v,i) => <div key={i} className={styles.vdItem}><span className={styles.vdLabel}>{v.label}</span><span className={styles.vdPtsV}>{v.coste} pts</span></div>)
                : <div className={styles.narrText} style={{fontSize:'7pt',color:'#7a5a30'}}>Ninguna</div>}
            </div>
            <div>
              <div className={styles.bloqueTitle}>Desventajas</div>
              {(pnj.desventajas||[]).length > 0
                ? pnj.desventajas.map((d,i) => <div key={i} className={styles.vdItem}><span className={styles.vdLabel}>{d.label}</span><span className={styles.vdPtsD}>{d.puntos} pts</span></div>)
                : <div className={styles.narrText} style={{fontSize:'7pt',color:'#7a5a30'}}>Ninguna</div>}
            </div>
          </div>
        </div>
        ...
        {pnj.secreto_inconfesable && (
          <div className={`${styles.habBlock} ${styles.secretoBlock}`} style={{top:'230mm'}}>
            <div className={styles.sectionTitle} style={{color:'#5a1010'}}>Secretos</div>
            {pnj.secreto_arquetipo && <div className={styles.bloqueTitle} style={{marginBottom:'1mm'}}>{pnj.secreto_arquetipo}</div>}
